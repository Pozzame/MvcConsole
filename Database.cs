...
class Database
{
    ...
    public List<User> GetUsers()
    {
        ...
        while (reader.Read())
        {
            users.Add(new User(reader.GetString(1), reader.GetInt32(0))); // Aggiunta dell'utente alla lista
        }
    ...
    {
        var command = new SQLiteCommand($"SELECT * FROM users WHERE name LIKE '%{name}%'", _connection);
        var reader = command.ExecuteReader();   // Esecuzione del comando e creazione di un oggetto per leggere i risultati
        var users = new List<User>(); // Creazione di una lista per memorizzare i nomi degli utenti
        while (reader.Read())
        {
